...
namespace SHOPLITE.ModalForms
{
    public partial class frmPurchaseOrder : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("No records to save.", "Shoplite", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            string returncode = "";
            int lponumber;
            ...
            if (returncode == "Success")
            {
                ...
                MessageBox.Show("Lpo Saved Successfully", "Shoplite", MessageBoxButtons.OK, MessageBoxIcon.Information);
                MessageBox.Show("Lpo Number " + lponumber + ".", "Shoplite", MessageBoxButtons.OK, MessageBoxIcon.Information);
                if (MessageBox.Show("Do you wish to print?", "Shoplite", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    printlpo();
                }
            ...
            {
                MessageBox.Show("Lpo Saving failed. please contact system administrator for assistance", "Shoplite", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txtProdCd.Text))
            {
                MessageBox.Show("Please Enter Product Code to continue", "Shoplite", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (string.IsNullOrEmpty(txtQty.Text))
            {
                MessageBox.Show("Please Enter Quantity to continue", "Shoplite", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            foreach (DataGridViewRow row in dgvPo.Rows)
            {
                ...
                {
                    MessageBox.Show("Product " + txtProdCd.Text + " is already added");
                    return;
                }
            }
            ProductRepository productRepository = new ProductRepository();
            ...
            if (product == null)
            {
                MessageBox.Show("Please Enter valid Product Code to continue", "Shoplite", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else
            {
        ...
        private void txtPoNo_Leave(object sender, EventArgs e)
        {
            ...
            if (order != null)
            {
                ...
                {
                    MessageBox.Show("Error Retrieving Lpo Record.", "ShopLite", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                MessageBox.Show("Invalid Lpo Number.", "ShopLite", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtPoNo.Text = "";
                btnCancel_Click(sender,e);
                txtPoNo.Focus();
            }
