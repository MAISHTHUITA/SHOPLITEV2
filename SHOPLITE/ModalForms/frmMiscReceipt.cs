...
{
    public partial class frmMiscReceipt : Form
    {
        public frmMiscReceipt()
        {
            InitializeComponent();
        }
        private static frmMiscReceipt _instance;
        public static frmMiscReceipt Instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new frmMiscReceipt();
                }
            ...
            if (RJMessageBox.Show("Are you sure you want to save.", "Save?", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    using (SqlConnection con = new SqlConnection(DbCon.connection))
                    {
                        ...
                        try
                        {
                            command.CommandText = @"insert into RECEIPTMASTER (REASON,USERNAME,NETAMOUNT,VATAMOUNT) values(@Reason,@Username,@TotalAmount, @VatAmount) SELECT SCOPE_IDENTITY()";
                            ...
                            {
                                command.CommandText = @"Declare @intqty decimal set @intqty=(select QtyAvble from tblProd where ProdCd=@prodcd) insert into RECEIPTDETAILS(ProdCd,ProdNm,UnitCd,Quantity,CP,SP,LineNetAmount,LineVatAmount,RECEIPTSRNO) values (@Prodcd,@ProdNm, @unit, @Quantity, @Cp, @Sp, @LineNetAmt, @LineVatAmt, @SrNo) update tblprod set QtyAvble = QtyAvble + @Quantity where ProdCd = @ProdCd  declare @Newqty decimal set @newqty =(@IntQty + @quantity) insert into tblProdHist (Prod_Cd,Txn_Type,QTY,Int_QTy,Nw_Qty,Prod_Cp,Prod_SP,Usr_Nm,DOC_NO,ACCOUNT_NO) values (@ProdCd,@Reason,@Quantity,@IntQty,@NewQty,@cp,@Sp,@Username,@SrNo,'Miscellenous Receipt')";
                            ...
                            RJMessageBox.Show("Receipt Saved Successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            /// if  we reach here, no error has occurred;
                            RJMessageBox.Show("The Miscellenous Receipt Note Number is " + returned);
                            if ((RJMessageBox.Show("DO YOU WANT PRINT?", "Print?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)) == DialogResult.Yes)
                            {

                                PrintMethod("ORIGINAL", "Miscellenous Receipt");
                            }
        ...
        {
            List<ReceiptDetails> grnDetails1 = new List<ReceiptDetails>();
            foreach (DataGridViewRow row in dgvgd.Rows)
            {
                ReceiptDetails grnDetails = new ReceiptDetails();
            ...
            ReceiptMaster grnMaster = new ReceiptMaster();
            TransactionsRepository transactions = new TransactionsRepository();
            grnMaster = transactions.GetReceiptMaster(Convert.ToInt32(txtSn.Text));
        ...
        private void txtSn_Leave(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection con = new SqlConnection(DbCon.connection))
                    {
                        SqlCommand cmd = new SqlCommand("Select * from Receiptdetails where Receiptsrno =@srno", con);
        ...
        private void txtReason_Leave(object sender, EventArgs e)
        {
            if (!String.IsNullOrEmpty(txtReason.Text))
            {
                ...
                else
                {
                    ...
                    txtReason.Enabled = false;
                    pnlgdproduct.Enabled = btngdSave.Enabled = true;
                    txtgdProdNm.Focus();
                }
            ...
            {
                RJMessageBox.Show("Please Enter Reason Code.");
                txtReason.Focus();
            }
        }
